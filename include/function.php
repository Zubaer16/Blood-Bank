...
function checkHospital($username){
    ...
    $query="select count(*) as count from ".prefix("hospital")." where username='{$username}'";  //checking existing user
...
function checkHospitalId($hospital_id){
    ...
    $query="select count(*) as count from ".prefix("hospital")." where hospital_id='{$hospital_id}'";  //cheking existing hospital
    $result=mysqli_query($con,$query);
    if($result){
        $row=mysqli_fetch_assoc($result);
...
function insert_hospital($username,$hospital_name,$password,$mobile){
    ...
    $status=1;
    $query="insert into ".prefix("hospital")." (username,hospital_name,password,mobile,datetime,status) values ('{$username}','{$hospital_name}','{$password}','{$mobile}','{$datetime}','{$status}') ";          //sign up hospital
    $result=mysqli_query($con,$query);
...
function insert_user($username,$first_name,$last_name,$password,$mobile,$blood_id){
    ...
    $datetime=date("Y-m-d H:i:s");
    $status=1;
    $query="insert into ".prefix("user")." (username,first_name,last_name,password,mobile,blood_id,datetime,status) values ('{$username}','{$first_name}','{$last_name}','{$password}','{$mobile}','{$blood_id}','{$datetime}','{$status}') ";    //sign up user
    $result=mysqli_query($con,$query);
    if($result) return true;
    else return false;
}
...
function checkBlood($blood_id){
    ...
    $query="select count(*) as count from ".prefix("blood")." where blood_id='{$blood_id}'";    //blood search
    $result=mysqli_query($con,$query);
    if($result){
        $row=mysqli_fetch_assoc($result);
        if($row["count"]==1) return true;
...
function checkStock($hospital_id,$blood_id,$volume){
    ...
    $query="select volume from ".prefix("stock")." where hospital_id='{$hospital_id}' and blood_id='{$blood_id}'";   //stock
    $result=mysqli_query($con,$query);
    if(mysqli_num_rows($result)==1){
        $row=mysqli_fetch_assoc($result);
...
function stockList($obj){
    ...
    $stock=array();
    $query="select s.stock_id,s.hospital_id,h.hospital_name,s.blood_id,b.blood,b.detail,s.volume from ".prefix("stock")." s left join ".prefix("blood")." b on s.blood_id=b.blood_id left join ".prefix("hospital")." h on s.hospital_id=h.hospital_id where ";  //join three tables stock, blood and hospital
...
function crypto($action, $string) {   //passwod generator
