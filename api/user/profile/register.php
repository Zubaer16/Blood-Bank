...
//User Login (For User Data Table)
...
if(!PRODUCTION){
	if(!empty($username) && !empty($first_name) && !empty($last_name) && !empty($_REQUEST["password"]) && !empty($_REQUEST["mobile"]) && !empty($_REQUEST["blood_id"])){
		if(strlen($username)>=4 && strlen($first_name)>=4 && strlen($last_name)>=4){    //Setting user name length
			if(strlen($_REQUEST["password"])>=6 && strlen($_REQUEST["password"])<=12){   // setting password
				if(strlen($mobile)==11 && strlen($_REQUEST["mobile"])==11){   //mobile number
					...
					$output='{"status":"failure", "remark":"Mobile must be 11 digit long"}';
				}
			}else{
